# ============================================================
# 07_proteomics_87_model.R  — proteomics-only model on the 87 cohort
# ------------------------------------------------------------
# Trains EN + RF on the 87 patients using ONLY the selected proteins
# (from 05_select_features). One of the three modelling approaches;
# 09_compare_performance will read this + 06 (+ 08) to compare.
#
# Proteins were selected on LCP2 (04/05), so the 87 stays a clean testbed.
# All 8 selected proteins are present in the 87 panel (matched by Assay name).
...
OUT    <- "results/07_proteomics_87"
SUFFIX <- ""                    # "_loose" for the sensitivity variable set
if (!dir.exists(OUT)) dir.create(OUT, recursive = TRUE)

## ---- build the reduced proteomics table --------------------------------------
# ml87_proteomics.csv is already in the canonical Patient order cv_folds87 uses.
...
if (length(setdiff(top_p, names(prot87))))
  warning("selected proteins not found in the 87 panel: ",
          paste(setdiff(top_p, names(prot87)), collapse = ", "))

reduced <- prot87 %>% dplyr::select(Patient, Lungcancer, dplyr::all_of(keep_p))
write.csv(reduced, file.path("processed_data", paste0("ml87_top_proteomics", SUFFIX, ".csv")),
          row.names = FALSE)
cat(sprintf("proteomics 87 table: %d patients x %d proteins (%d requested)\n",
            nrow(reduced), length(keep_p), length(top_p)))
...
# Same 87 patients/order/label as 06/08 + same seed => identical folds across the
...
res_en <- run_one(train, "EN", "EN · proteomics")
res_rf <- run_one(train, "RF", "RF · proteomics")
